...
- vision service runs the `price_tag_cpu_v1` skeleton pipeline by default
- the pipeline reads video metadata, samples a bounded set of frames, writes debug frame artifacts, and returns zero detections
- mock pipeline is still available as a fallback runtime
